  ...
  if (get("spositems")) {
    ...
    $.each(spositems, function () {
      ...
      tr_html +=
        ...
        "</span></td>";
      tr_html +=
        '<td class="text-center"><i class="fa fa-trash-o tip pointer posdel" id="' +
        row_no +
        '" title="Remove"></i></td>';
      newTr.html(tr_html);
      newTr.prependTo("#posTable");
      total += formatDecimal(
        (parseFloat(net_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty),
        4
      );
...
$(document).ready(function () {
  ...
  $(document).on("click", ".suspended_sales .dropdown-menu .header", function (
    e
  ) {
    e.stopPropagation();
  });
